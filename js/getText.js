  ...
  xhr.onreadystatechange = function() {
    ...
    if (xhr.status == 200) {
      try {
        ...
        function repGreen(value) {
          return value.replace(/\$\S+/g, function(match) {
            return '<span style="color: green;">' + match.slice(1) + '</span>';
          });
        }
        ...
        function showWord() {
          ...
          if (this.innerHTML === '____') {
            this.innerHTML = word;
          } else {
            this.innerHTML = '____';
          }
          const childWords = this.querySelectorAll('.word');
          childWords.forEach(childWord => {
            childWord.addEventListener('click', showWord);
          });
        }
